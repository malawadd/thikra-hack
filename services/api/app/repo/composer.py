...
def _scene_track_urls(
    b2_run, scene_count: int
    ...
    if not speech:
        ...
        music = (
            _asset_url_or_none(b2_run.run.steps[-1])
            if len(b2_run.run.steps) > scene_count * 2
            else None
        )
    else:
...
def _group_scenes(b2_run, b1_run, spec: StoryboardSpec, tmp: Path) -> list[_SceneBundle]:
    ...
    for i, scene in enumerate(spec.scenes):
        ...
        if video_url is not None:
            ...
            logger.info(
                "scene video fell back to keyframe still",
                extra={
                    "scene_index": i,
                },
            )
...
def _concat_video(
    scenes: list[_SceneBundle], tmp: Path, canvas: tuple[int, int] = _DEFAULT_CANVAS
) -> Path:
...
def _available_filters() -> frozenset[str]:
    ...
    return frozenset(
        m.group(1) for line in out.splitlines() if (m := re.match(r"\s*[TSC.]{3}\s+(\S+)\s", line))
    )
...
def _finalize_with_captions(
    ...
    try:
        ...
        logger.warning(
            "caption finalize failed; producing an uncaptioned MP4",
...
def compose_final(
    b2_run, b1_run, spec: StoryboardSpec, canvas: tuple[int, int] = _DEFAULT_CANVAS
    ...
    with tempfile.TemporaryDirectory(prefix="genblaze-compose-") as tmp_str:
        ...
        try:
            ...
            logger.warning(
                "manifest embed failed (continuing without)",
                extra={
                    "run_id": run_id,
                    "exception": str(exc),
                },
...
def prepare_studio_asset(source: str, content_type: str, cache: Path) -> dict:
    ...
    duration = float(
        info.get("format", {}).get("duration") or (video or audio or {}).get("duration") or 0
    )
        ...
        if not thumb.exists():
            _run_ffmpeg(
                [
                    ...
                    str(original),
                    "-filter_complex",
                    "showwavespic=s=1200x180:colors=6fe7c8",
                    "-frames:v",
                    "1",
                    str(thumb),
                ],
...
def create_demo_editor_clip(destination: Path, duration_sec: float) -> Path:
    ...
    _run_ffmpeg(
        [
            ...
            "-vf",
            f"fade=t=in:st=0:d=.35,fade=t=out:st={max(0.35, duration_sec - 0.35)}:d=.35",
            "-c:v",
            "libx264",
            "-pix_fmt",
            "yuv420p",
...
def extract_studio_sequence_audio(
    ...
    for clip_index, clip in enumerate(document["clips"]):
        ...
        inputs += [
            "-ss",
            str(clip.get("source_in_ms", 0) / 1000),
            "-t",
            str(length),
            "-i",
            str(path),
...
def _text_card(clip: dict, canvas: tuple[int, int], destination: Path) -> Path:
    ...
    box = draw.multiline_textbbox(
        (0, 0), text, font=font, align=settings.get("align", "center"), spacing=8
    )
    ...
    if background[-2:] != "00":
        draw.rounded_rectangle((0, 0, image.width - 1, image.height - 1), 12, fill=background)
    draw.multiline_text(
        (padding_x - box[0], padding_y - box[1]),
        text,
        font=font,
        fill=settings.get("color", "#ffffff"),
...
def _srt_from_clips(clips: list[dict]) -> bytes | None:
    captions = sorted(
        (clip for clip in clips if clip["kind"] == "caption"), key=lambda item: item["start_ms"]
    )
    if not captions:
        return None
    lines: list[str] = []
    for index, clip in enumerate(captions, 1):
        ...
        lines.extend(
            [str(index), f"{_srt_ts(start)} --> {_srt_ts(end)}", clip["text"]["content"], ""]
        )
    ...
    *,
    ...
    if document.get("schema_version", 1) == 1:
        ...
        for clip in document.get("clips", []):
            if clip.get("kind") in {"text", "caption"} and clip.get("text"):
                clip.setdefault("transform", {})["position_x"] = clip["text"].get("position_x", 0.5)
    ...
    with tempfile.TemporaryDirectory(prefix="thikra-editor-render-") as tmp_str:
        ...
        input_args: list[str] = []
        filters = [
            f"color=c={document.get('background', '#05070a')}:s={canvas[0]}x{canvas[1]}:r=30:d={duration_ms / 1000}[base]"
        ]
        visual_labels: list[tuple[dict, str]] = []
        audio_labels: list[tuple[str, str]] = []
        input_index = 0
        ordered = sorted(
            active, key=lambda item: (tracks[item["track_id"]]["order"], item["start_ms"])
        )
        for clip_index, clip in enumerate(ordered):
            ...
            suffix = (
                Path(
                    urlparse(descriptor.get("url") or descriptor.get("path") or "source.bin").path
                ).suffix
                or ".bin"
            )
            path = _localize(
                descriptor.get("path") or descriptor.get("url"),
                tmp / f"source-{clip_index}{suffix}",
            )
            ...
            if clip["kind"] in {"video", "image"}:
                ...
                normalize = (
                    ...
                    + (
                        ...
                        if fit == "fill"
                        else f"format=rgba,pad={canvas[0]}:{canvas[1]}:(ow-iw)/2:(oh-ih)/2:color=black@0"
                    )
                )
                ...
                fade_in = min(
                    clip["duration_ms"] // 2,
                    int(
                        transform.get("fade_in_ms", 0)
                        or (transition_ms if transition_in != "cut" else 0)
                    ),
                )
                fade_out = min(
                    clip["duration_ms"] // 2,
                    int(
                        transform.get("fade_out_ms", 0)
                        or (transition_ms if transition_out != "cut" else 0)
                    ),
                )
                if fade_in:
                ...
                if (
                    clip["kind"] == "video"
                    and descriptor.get("has_audio")
                    and not tracks[clip["track_id"]].get("muted")
                ):
                    audio = clip.get("audio") or {}
        ...
        for overlay_index, (clip, label) in enumerate(visual_labels):
            ...
            transform = clip.get("transform") or {}
            x = float(transform.get("position_x", 0.5))
            y = float(transform.get("position_y", 0.5))
            filters.append(
                f"[{current}][{label}]overlay=W*{x}-w/2:H*{y}-h/2:eof_action=pass:enable='between(t,{start},{end})'[{next_label}]"
            )
            current = next_label
        if audio_labels:
            ...
            if document.get("duck_music_under_narration") and narration and music:
                ...
                filters.append(
                    "[musicmix][narrside]sidechaincompress=threshold=.08:ratio=8:attack=20:release=500[ducked]"
                )
