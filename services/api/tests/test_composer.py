...
SCENE_URLS = [f"{_B2_HOST}/{_RUN_PREFIX}/step-{i:02d}/scene.mp4" for i in range(4)]
NARRATION_URLS = [f"{_B2_HOST}/{_RUN_PREFIX}/step-{i:02d}/narration.wav" for i in range(4)]
KEYFRAME_URLS = [f"{_B2_HOST}/{_RUN_PREFIX}/kf-{i:02d}/keyframe.png" for i in range(4)]
MUSIC_URL = f"{_B2_HOST}/{_RUN_PREFIX}/step-music/track.wav"


def _make_spec(n: int = 4) -> StoryboardSpec:
...
def _make_b1_run(n: int = 4, *, keyframe_scenes: set[int] | None = None):
...
def test_group_scenes_pairs_video_and_narration(tmp_path: Path) -> None:
    spec = _make_spec(4)
    b2_run = _make_b2_run(4)
    b1_run = _make_b1_run(4)
    fake = MagicMock()
    fake.get.side_effect = lambda key: b"fake-bytes"
    with patch.object(composer, "backend", return_value=fake):
    ...
    assert "scene_03_voice.wav" in bundles[3].narration_path.name
...
def test_group_scenes_falls_back_to_keyframe_when_video_missing(tmp_path: Path) -> None:
    ...
    spec = _make_spec(4)
    b2_run = _make_b2_run(4, video_scenes={0, 1, 3})  # scene 2's video failed
    b1_run = _make_b1_run(4)
    fake = MagicMock()
    fake.get.side_effect = lambda key: b"fake-bytes"
    with patch.object(composer, "backend", return_value=fake):
...
def test_group_scenes_raises_when_video_and_keyframe_both_missing(tmp_path: Path) -> None:
    ...
    spec = _make_spec(4)
    b2_run = _make_b2_run(4, video_scenes={0, 1, 3})       # scene 2 video failed
    b1_run = _make_b1_run(4, keyframe_scenes={0, 1, 3})    # scene 2 keyframe also missing
    fake = MagicMock()
    fake.get.side_effect = lambda key: b"fake-bytes"
    with patch.object(composer, "backend", return_value=fake), \
...
def test_compose_final_writes_to_b2_under_explainers_prefix() -> None:
    spec = _make_spec(4)
    asset, notices, calls = _run_compose(_make_b2_run(4), spec)
...
def test_compose_final_music_unavailable_mixes_narration_only() -> None:
    ...
    spec = _make_spec(4)
    asset, notices, calls = _run_compose(_make_b2_run(4, music=False), spec)
    ...
    assert "amix=inputs=4" in mix      # four narration tracks only
    assert asset.media_type == "video/mp4"
...
def test_compose_final_music_only_when_all_narration_failed() -> None:
    ...
    spec = _make_spec(4)
    _, notices, calls = _run_compose(_make_b2_run(4, narration_scenes=set()), spec)
...
def test_compose_final_partial_narration_indexes_added_inputs() -> None:
    ...
    spec = _make_spec(4)
    # Scenes 1 and 3 (0-indexed) lost narration → kept: 0, 2.
    _, notices, calls = _run_compose(_make_b2_run(4, narration_scenes={0, 2}), spec)

    assert notices == ["Narration unavailable for scene(s) 2, 4."]
    mix = _filter_for(calls, "mix-audio")
...
def test_compose_final_no_audio_renders_silent_video() -> None:
    ...
    asset, notices, calls = _run_compose(
        _make_b2_run(4, narration_scenes=set(), music=False), spec,
    )
...
def test_compose_final_soft_subs_when_libass_missing() -> None:
    ...
    spec = _make_spec(4)
    asset, notices, calls = _run_compose(_make_b2_run(4), spec, has_subtitles=False)
...
def test_compose_final_video_falls_back_to_keyframe_still() -> None:
    ...
    spec = _make_spec(4)
    asset, notices, calls = _run_compose(_make_b2_run(4, video_scenes={0, 1, 3}), spec)
...
def test_compose_final_fails_loud_when_ffmpeg_missing() -> None:
    with patch.object(composer.shutil, "which", return_value=None), \
        ...
        composer.compose_final(_make_b2_run(4), _make_b1_run(4), _make_spec(4))
