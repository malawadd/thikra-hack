<script lang="ts">
  ...
  let timeline = $state<any[]>([]); let graph = $state<any>({ nodes: [], edges: [] }); let loading = $state(true); let error = $state(''); let tab = $state<'timeline'|'graph'>('timeline'); let selected = $state<any>(null); let group = $state('all');
  const runId = page.url.searchParams.get('run');
  const visibleNodes = () => group === 'all' ? graph.nodes : graph.nodes.filter((node:any) => !node.group || node.group === group);
  const visibleEdges = () => { const ids = new Set(visibleNodes().map((node:any) => node.id)); return graph.edges.filter((edge:any) => ids.has(edge.source) && ids.has(edge.target)); };
  const point = (id: string) => { const index = visibleNodes().findIndex((node: any) => node.id === id); return { x: 120 + (index % 4) * 225, y: 75 + Math.floor(index / 4) * 145 }; };
  function inspect(node:any) { selected = node; if (node.url) void goto(node.url); }
...
<AsyncState {loading} {error} empty={!loading && timeline.length === 0}>
  ...
  {:else}
    <div class="actions" style="margin-bottom:14px">{#each ['all','commercial','payment','fulfillment','media','risk','redress'] as value}<button class="btn btn-secondary" class:btn-primary={group===value} onclick={()=>group=value}>{titleCase(value)}</button>{/each}</div>
      ...
      <div class="card graph-wrap" aria-label="Evidence relationship graph">
        <svg viewBox="0 0 950 600" role="img">
          ...
          {#each visibleNodes() as node}
            ...
            <g class="graph-node" class:selected={selected?.id === node.id} transform={`translate(${p.x},${p.y})`} onclick={() => inspect(node)} role="button" tabindex="0" onkeydown={(event) => event.key === 'Enter' && inspect(node)}>
