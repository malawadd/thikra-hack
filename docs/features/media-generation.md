...
Installed Studio generation runs Genblaze without a mandatory object-storage sink and immediately ingests each provider result into hash-addressed local project storage. If B2 is connected in Windows Credential Manager, Studio also writes a durable copy through `genblaze-s3`. Image-to-video reference handoff is the deliberate exception: an external provider cannot fetch a loopback file, so that operation preflights the optional B2 connection and presigns the uploaded reference. Text-only generation and all editing remain available without B2. The original web/commerce B1/B2 sink and lineage behavior below is unchanged.
